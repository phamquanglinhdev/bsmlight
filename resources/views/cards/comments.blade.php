    ...
    <div class="app-chat-history">
        <div class="chat-history-body rounded ps ps--active-y" style="overflow-y: scroll!important;">
            <ul class="chat-history">
                @foreach($comments as $comment)
                    @if($comment->isSelf())
                        @include('cards.self-comment',['comment' => $comment])
                    @else
                        @include('cards.comment',['comment' => $comment])
                    @endif
                @endforeach
            </ul>
        </div>
        <div class="chat-history-footer p-0 mt-4">
            <form id="send-comment" method="POST" action="{{url('comment/store')}}"
                ...
                @csrf
                <input type="hidden" name="object_id" value="{{$card->getOriginal('id')}}">
                <input type="hidden" name="object_type" value="{{Comment::CARD_COMMENT}}">
